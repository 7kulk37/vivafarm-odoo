"""account.move extensions — customer invoice acknowledgment hash.

When the customer acknowledges an invoice via the portal (ใบแจ้งหนี้
commercial invoice, EVIDENCE-ONLY per lawyer sign-off 2026-08-19), the
...
customer acknowledged as the amount due.

The acknowledgment is OPTIONAL and NEVER gates payment: refusal = chatter
note + no state change (the invoice stays posted/not_paid). The signed
Delivery Note remains the primary acceptance evidence.

Lock: once hashed, substance changes (partner, lines, amounts, dates,
terms) are rejected at ORM level — the acknowledged invoice is frozen.
Correction = void + reissue (ป.86/2542 ข้อ 25), never an edit.
"""
...
#: Fields that define the financial substance of an acknowledged invoice.
#: If any of these change after the customer's acknowledgment is hashed,
#: the evidence no longer matches the acknowledged amount.
PROTECTED_FIELDS = {
